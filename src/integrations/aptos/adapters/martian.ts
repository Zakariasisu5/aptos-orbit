// Martian adapter (lightweight wrapper)
...
export const isAvailable = () => {
  try {
    // @ts-ignore
    return typeof window !== 'undefined' && !!(window as any).martian;
  } catch (e) {
    return false;
  }
};
...
export async function connect() {
  ...
  if (!provider) throw new Error('Martian provider not found');
...
export async function disconnect() {
  try {
    ...
    console.warn('Martian disconnect error', e);
  }
}
...
export async function signAndSubmit(payload: any) {
  ...
  if (!provider) {
    ...
    throw new Error('Martian provider not available');
  }
  ...
  throw new Error('No supported signAndSubmit method found on Martian provider');
}
